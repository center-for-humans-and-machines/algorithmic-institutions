...
def analyse(args):
    ...
    for name, (fam, legal, desc) in CANDIDATES.items():
        ...
        say(
            ...
            f"lvl {shares['lvl_pearson']:+.3f} [{row['share_lvl_pearson_lo']:+.3f},{row['share_lvl_pearson_hi']:+.3f}]  "  # noqa: E501
            f"mle {shares['mle']:+.3f} [{row['share_mle_lo']:+.3f},{row['share_mle_hi']:+.3f}]"  # noqa: E501
            + (
    ...
    # ------------------------------------------------------------ forward selection
    say(
        "\n=== 3. forward selection over the legal candidates (criterion: pit_moment) ==="  # noqa: E501
    )
    ...
    while remaining:
        ...
        say(
            f"step {len(chosen)+1}: +{best:<20} rho {cur:.4f} -> {best_val:.4f}  drop {cur-best_val:+.4f} [{lo:+.4f},{hi:+.4f}]  {'KEEP' if lo > 0 else 'STOP (CI includes 0)'}"  # noqa: E501
        )
        if lo <= 0:
            break
        chosen.append(best)
    ...
    for label, z in (("before partialling", sc["z"]), ("after joint set", z_after)):
        ...
        # static vs round-local: one-way ICC of the cell mean across rounds within (game, group)  # noqa: E501
...
def figures(cand_tab, split_tab, pers_tab, full):
    ...
    ax.set_xlabel(
        "share of residual co-movement explained, 1 - rho_after / rho_before (95% game bootstrap)"  # noqa: E501
    )
    ax.set_title(
    ...
    for k, (_, row) in enumerate(pers_tab.iterrows()):
        ys = [row["rho_within"]] + [
            row[f"rho_cross_lag{l}"] for l in lags[1:]  # noqa: E741
        ]
