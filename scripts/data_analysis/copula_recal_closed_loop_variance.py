...
Ported from `scripts/data_analysis/copula_closed_loop_variance.py` (the
parent, `auto/sim-timeout-imputation`, itself a port of PR #188) unchanged
except for the arms it reads and where it writes: same measurements, same
conventions, same teacher-forced BARE trunk, so `var_cond_mean` here is the
same quantity that branch and PR #186 report against the human
`Var(E[c | history])` of 27.9. The arms isolate the recalibration:

  R  recalibrated copula on  -- 23_..._gnn_switch_copularecal (this branch)
  B  parent, copula on       -- 23_..._gnn_switch_simtimeout (the baseline)
  C  copula OFF              -- 23_2g8a_copula_recal_rho0 (the bare trunk)

Nothing is retrained in any arm; R and B run the same weights and differ only
in the stamped `copula_rho`. C loads the bare trunk, which no stamping ever
touches, so it is the arm a recalibration cannot reach by construction --
that is the point of running it.

Two stages, because the trunk unpickles torch_geometric modules:

  Raven:  .venv/bin/python \\
...
OUT = ROOT / (
    "plots/data_analysis/evaluation/contribution_copula_recalibrated"
    "/copula_closed_loop"
)
...
ARMS = {
    "R": SKIP + "_copularecal",  # recalibrated copula on (this branch)
    "B": SKIP + "_simtimeout",  # parent, copula on (the baseline)
    "C": "23_2g8a_copula_recal_rho0",  # copula OFF, bare trunk
}
ZLOG = "23_2g8a_copula_cl_variance_a_zlog"  # a latent-logged rerun; none here
LABELS = {"human": "human", "R": "R recalibrated, copula on",
          "B": "B parent, copula on", "C": "C copula OFF"}  # fmt: skip
...
def latent_regression():
    """Latent-logged rerun (none on this branch): regress the group mean
    ...
    src = pd.read_parquet(SIM / ARMS["R"] / "per_round.parquet")
    same = (
...
def plot(per_round):
    ...
    palette = {"human": "#222222", "R": "#d62728", "B": "#1f77b4",
               "C": "#ff7f0e"}  # fmt: skip
    panels = [
    ...
    fig.suptitle(
        "Closed-loop variance, copula recalibrated (stimulus-skip trunk, "
...
def analyse():
    ...
    with open(OUT / "tables.md", "w") as f:
        ...
        if lat is not None:
            f.write("## group mean on the logged latent\n\n")
