...
def gain_gaussian():
    ...
    for key in BUNDLES:
        ...
        def realised_shift(d, delta, sel):
            ...
            pool = build_feature_pool(shifted(d, delta, b["default_values"]["contribution"]), b["switch_every"])  # noqa: E501
            f = "prev_contribution_mean_group"
            return {"group_feature_shift": float((pool[f][mask] - pool0[f][mask])[sel].mean() / delta)}  # noqa: E501

        tabs.append(
            gain_table(evaluate, data, b["default_values"]["contribution"], key, realised_shift)  # noqa: E501
        )
...
def analyse():
    ...
    pd.concat(per_round.values(), keys=per_round.keys(), names=["arm"]).reset_index().to_csv(  # noqa: E501
        OUT / "per_round.csv", index=False
    )
    ...
    with open(OUT / "tables.md", "w") as f:
        ...
        n = gain[gain["set"] == "own_in_grid"].pivot(index="delta", columns="model", values="n")  # noqa: E501
        f.write("## Rows per delta (own_in_grid)\n\n" + ccv.md_table(n.reset_index(), "{:.0f}") + "\n")  # noqa: E501
