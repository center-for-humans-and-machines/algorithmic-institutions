...
def main():
    ...
    for case, label, src, run, gated in CASES:
        before = read_scores(os.path.join(SIM, src + "_ceiling", "evaluation/scores.csv"), run)
        after = read_scores(os.path.join(SIM, src + "_timeout", "evaluation/scores.csv"), run)
        ...
        t.loc["mean"] = [sb["mean"], sa["mean"], d_mean, "", MEAN_SEED_SD,
                         ...
                         abs(d_mean) >= MEAN_SEED_SD, False]
        t.loc["rows <= 1"] = [sb["rows <= 1"], sa["rows <= 1"], d_rows, "",
                              ROWS_LE1_SEED_SD,
                              round(abs(d_rows) / ROWS_LE1_SEED_SD, 2),
                              abs(d_rows) >= ROWS_LE1_SEED_SD, True]
        t.index.name = "row"
        rce_rows, checks = rce_check(
            human_fit, src + "_ceiling", src + "_timeout", run, before["RCE"], after["RCE"]
        )
        ...
        if gated:
            ...
            verdict = dict(
                ...
                mean_move_in_seed_sd=round(abs(sa["mean"] - sb["mean"]) / MEAN_SEED_SD, 2),
