...
def load():
    ...
    df = df.sort_values(["episode_id", "player_id", "round_number"]).reset_index(
        drop=True
    )
    # per-player lags
    ...
    gmean = df.groupby(["episode_id", "round_number", "group_id"])[
        "contribution"
    ].mean()
    cg = df.groupby(["episode_id", "round_number", "group_id"])["common_good"].first()
    gmean_d, cg_d = gmean.to_dict(), cg.to_dict()
    gsum_d = (
        df.groupby(["episode_id", "round_number", "group_id"])["contribution"]
        .sum()
        .to_dict()
    )
    gcnt_d = (
    ...
    for ep, r, pg, sc in df[
        ["episode_id", "round_number", "prev_group", "self_prev_c"]
    ].values:
        if pd.isna(pg):
            own_prev.append(np.nan)
            oth_prev.append(np.nan)
            own_cg_p.append(np.nan)
            oth_cg_p.append(np.nan)
            continue
        pg = int(pg)
        pr = r - 1
...
def main():
    ...
    dec = df[
        df["round_number"].isin(DECISIONS) & (df["selection_timeout"].fillna(0) == 0)
    ].copy()
    ...
    fig.tight_layout()
    fig.savefig(OUT / "drivers.png", dpi=130)
    ...
    for ep, r, pg, gid, sc, c, peers in sw_rows.values:
        rows.append(
            dict(
                ...
                new_pre=gmean.get(
                    (ep, r - 1, int(gid))
                ),  # joined group level, pre-decision
                old_pre=gmean.get(
                    (ep, r - 1, int(pg))
                ),  # left group level, pre-decision
            )
        )
    ...
    print(
        f"     new-group peers   {b4['new_peers']:+.3f}   (>0 => adopts new group's norm)"
    )
    print(
        f"   raw: switchers post mean={ADr['post'].mean():.2f}, "
    ...
    cgb = (
        df.groupby(["episode_id", "round_number", "group_id"])["common_good"]
        .first()
    ...
    print(
        f"   mean contribution round 0={mc.iloc[0]:.2f}, mid={mc.loc[12]:.2f}, "
        f"last(23)={mc.iloc[-1]:.2f}, drop 22->23={mc.iloc[-1]-mc.loc[22]:+.2f}"
    )
    print(f"   switch rate first(4)={sw.loc[4]:.1%} -> last(20)={sw.loc[20]:.1%}")

    fig, ax = plt.subplots(1, 3, figsize=(13, 3.4))
    ax[0].plot(mc.index, mc.values, "-o", ms=3)
    ax[0].set_title("Mean contribution")
    ax[0].set_xlabel("round")
    ax[0].axvline(23, color="grey", ls=":", lw=0.8)
    for d in DECISIONS:
        ax[0].axvline(d, color="orange", ls=":", lw=0.5)
    ax[1].plot(mp.index, mp.values, "-o", ms=3, color="#d62728")
    ax[1].set_title("Mean punishment")
    ax[1].set_xlabel("round")
    ax[2].plot(sw.index, sw.values, "-o", ms=4, color="#9467bd")
    ax[2].set_title("Switch rate (decisions)")
    ax[2].set_xlabel("round")
    ax[2].set_ylim(0, None)
    fig.suptitle(
        "Evolution over rounds (orange = switch decisions, dotted grey = final round)",
        fontsize=11,
    )
    fig.tight_layout()
    fig.savefig(OUT / "evolution.png", dpi=130)
    print(f"saved {OUT/'evolution.png'}")
    ...
    print(
        f"   punishment(t) {bd['punishment']:+.3f}  (>0 => punishment raises next contribution)"
    )
    ...
    det2 = det.copy()
    det2["pbin"] = pd.cut(
        det2["punishment"], [-0.1, 0.1, 5, 15, 31], labels=["0", "1-5", "6-15", "16-30"]
    )
    g = det2.groupby("pbin", observed=True)["dc"].mean()
    ax[0].bar(g.index.astype(str), g.values, color="#1f77b4")
    ax[0].axhline(0, color="k", lw=0.6)
    ax[0].set_title("Punishment -> next-round Δcontribution")
    ax[0].set_xlabel("punishment received")
    ax[0].set_ylabel("Δ contribution")
    ax[1].bar(
        ["left group", "joined group"],
        [smart["old_pre"].mean(), smart["new_pre"].mean()],
        color=["#d62728", "#2ca02c"],
    )
    ax[1].set_title("Switchers: quality of group left vs joined")
    ax[1].set_ylabel("mean contribution")
    fig.tight_layout()
    fig.savefig(OUT / "bonus.png", dpi=130)
    print(f"saved {OUT/'bonus.png'}")
