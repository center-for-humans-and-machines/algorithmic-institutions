...
from the CSVs of the punisher re-baseline (worktree D) and the RCE comparison
(worktree R), plus the committed analysis files of the seven PRs that landed
after the re-baseline, read straight off their branches with gitfile() and
cached beside this script.

The six September measurement PRs (#183, #186, #187, #188, #191, #195) were
consolidated onto main, so their files -- including the held-out teacher-forced
table that used to be read out of a scratch worktree -- are read from SEPT
below rather than from six experiment branches. Set SEPT_BRANCH to read them
from the consolidation branch while that PR is still open.
"""
import ast, base64, csv, html, io, json, math, os, re, statistics, subprocess
...
ho = pd.read_csv(gitfile(SEPT, "plots/data_analysis/rcb_holdout_teacher_forced/group_vnode_raven_gpu.csv"))
...
# #186, #187, #188 and #191 now live on SEPT (the September consolidation), not on
# their experiment branches; #192 and #190 are still read off theirs.
...
B_SS, B_TO = SEPT, "auto/punisher-timeout-feature"  # #195 consolidated, #196 not
