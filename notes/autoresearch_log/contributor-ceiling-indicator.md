...
8. **Housekeeping.** The isolated remote dir `~/repros/ai-runs/contributor-ceiling` can be deleted when this PR closes. The test suite in an isolated dir needs `plots/simulation/22_2g8a_linear_self_ridge_contr/per_round.parquet` shipped by hand (`rsync` excludes `plots/`), or nine evaluation-suite tests fail on the missing fixture; with it shipped, the first run was 133 of 134 and the one failure was this branch's own test asserting the masked target under the wrong name (`mask_data` writes `contribution_masked`, it does not overwrite `contribution`), fixed in `ab43aea`; the final run is **134 passed**. Black and flake8 clean under `src/`.
