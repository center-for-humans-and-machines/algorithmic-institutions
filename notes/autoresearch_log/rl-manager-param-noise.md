...
**The rival explanation, now settled by a separate probe and not by this arm.** The worry was that the artificial humans respond to punishment regardless of whether it was deserved, in which case no targeting choice changes the return, the contingency is unidentified by the reward, and it is arbitrary for reasons that have nothing to do with exploration.

`auto/contributor-punishment-targeting` (PR #215) tested it causally rather than by inference from outcomes: it intervened on the contributor model directly, forcing punishment at fixed contribution levels and reading the response. **The strong form is rejected.** The contributors' response to punishment does depend on their own contribution, in the human direction, in all six retrains, five seed standard deviations clear of zero, with the human surface shape — compliance below, withdrawal above, the sign flipping near contribution 12.

**But the gradient is about a fifth of the human one.** Moving a punishment point from a full contributor to a free-rider buys 0.066 against the humans' 0.301; 21.8% with contribution fixed effects inside the band; **8.6% restricted to the states the model actually reaches**. At the ceiling, where the inverted seeds aim, punishing a full contributor moves its next contribution by −0.246 against the humans' −7.035 — not free, since the pool still pays for it, but close to behaviourally inert.

So the signal exists and is correctly signed: a manager here is not optimising in a world where aim is free. It is simply **small against the reward noise**, which is exactly what four arms failing while payoff tracks force alone looks like. This does not excuse an exploration failure and it does not diagnose one; it bounds how much aim could have been worth.
...
1. **That the shape result is an exploration failure at all.** No longer wholly inferred: PR #215 has since measured the targeting gradient causally and found it real, correctly signed, and about a fifth of the human size — 8.6% in the states the model actually reaches. So the replay buffer's contingency is not irrelevant, and this arm's failure to move the shape is not explained away. What remains inferred is the attribution: whether four arms failed because the gradient is small against the reward noise, or because coherent exploration is the wrong lever, is not separated by anything measured here.

   **And one thing the probe could not settle, carried rather than resolved.** There is no interventional human. Comparing a *forced* model slope against an *observed* human slope is not like for like: if the human figure is itself mostly the state that the punishment co-occurred with rather than its effect, then the model may be faithful and the world may simply offer less targeting gradient than the metric implies. On that reading the ratio of one-fifth is an artefact of the comparison, not a defect of the clone. Nothing in this campaign can currently tell those apart.
...
## A check the other three arms' tables need

**`group_payoff_sum` rewards retention, not welfare, and three arms are already written up.** It is a sum over whoever is in the group, and membership in this env is endogenous: contributors switch groups in response to punishment, so a manager that merely holds on to more players scores higher on the sum with nobody better off.

Measured on this arm's five seeds, the correction is an order of magnitude:

| quantity | span across the five seeds |
|---|---|
| `group_payoff_sum` minus the clone's | **−40.2 to +30.6** |
| the same **per member** | **−4.06 to +1.99** |

Group sizes ran from 3.415 to 4.480 for the RL manager against 3.520 to 4.585 for the clone, out of eight players. Seed 46's apparent +30.6 is mostly that it held 4.48 members to the clone's 3.52; per member it is +1.99. Seed 42's −40.2 is −4.06.

Any arm whose payoff comparison uses a summed group quantity should divide by `rl_avg_group_size` / `opp_avg_group_size`, both of which are already in the metrics schema, or read `common_good`, which is per-capita by construction. This is not a subtlety about magnitudes: it can reorder managers, because retaining players and making them better off are different achievements and punishment drives both.
...
**And the finding that cuts across all four arms is in the cost table, not the shape table.** Payoff in this world is a near-deterministic decreasing function of how much the manager punishes (r = −0.974 over my five seeds) and is unrelated to whom it punishes.

PR #215 has since measured why, causally. The targeting gradient is **real and correctly signed** — forcing punishment at fixed contribution levels moves the contributors' next contribution in the human direction in all six retrains, five seed standard deviations clear of zero — so the strong rival explanation, that aim is free and the contingency unidentified, is **rejected**. But the gradient is about a fifth of the human one: 0.066 against 0.301 for moving a punishment point from a full contributor to a free-rider, and **8.6% of the human slope restricted to the states the model actually reaches**. At the ceiling, where the inverted seeds aim, punishing a full contributor moves its next contribution by −0.246 against the humans' −7.035.

That is the quantitative version of what my cost table shows from the outside. Aim is worth something and worth very little, and it has to be bought with punishment that the pool pays for at full price. **s43 is this arm's single instance of the trade being taken**: the one seed that moved contributions, by 0.94, and the gain did not cover the 1.507 extra punishment it spent. A gradient at 8.6% of human, competing against a cost at 100%, is a signal that four independent exploration methods failed to climb — and that is a statement about the world these managers are trained in, not only about exploration.
...
3. **The desert question is answered; do not re-run it.** `auto/contributor-punishment-targeting` (PR #215) intervened on the contributor model directly and found the targeting gradient real, correctly signed and human-shaped, but about a fifth of the human size and **8.6% of it in the states the model actually reaches** — −0.246 against −7.035 at the ceiling. The strong rival explanation is rejected; the weak one, that aim is worth too little to find against the reward noise, is quantified. s43 is this arm's one instance of the trade being taken and not covering its cost.

   What is *not* answered, and is the live question: **there is no interventional human**, so a forced model slope against an observed human slope is not like for like. If the human 0.301 is mostly the state that punishment co-occurred with rather than its effect, the clone is faithful and the world simply offers less gradient than the metric implies. Settling that needs an interventional human measurement or an observational estimator applied to the model on the same footing as the human — and it decides whether the campaign has been asking managers to find something that is barely there.
4. **Apply the membership correction to the three arms already written up.** `group_payoff_sum` rewards retention rather than welfare; on this arm's seeds the correction moved the span from −40.2–+30.6 to −4.06–+1.99. Divide by `rl_avg_group_size` / `opp_avg_group_size`, both already in the schema, or read `common_good`. It can reorder managers. See *A check the other three arms' tables need*.
