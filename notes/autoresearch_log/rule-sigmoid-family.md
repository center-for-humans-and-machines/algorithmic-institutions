...
**Both of those corrected the surrogate rather than confirming it.** `surrogate_profiles.jpg` shows the pool's posterior mean rising monotonically in `gamma_ep` right up to the box edge and the contribution's falling monotonically to it, so on the surrogate alone one would expect both to keep going in the same direction. Measured, the pool does not -- 4.0 and 6.0 are both slightly worse than 3.0 -- and the contribution does, by 1.30. **A fitted optimum on a box edge is a statement about the box; only the probes outside it are a statement about the family.**
