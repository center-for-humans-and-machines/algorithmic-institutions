...
6. (Added after review) Close the switch-model confound by measuring both switch-matched categorical cells (§3.7).
...
### 3.7 The switch-matched categorical arms (added after review)

§3.4's categorical rows ran with `switch_joint_exodus` and every Gaussian row with `switch_exodus_k_onehot`, which left the switch model as the one cross-lineage difference. Both matched cells now exist, so the comparison is like-for-like.

The copula-**on** cell already existed as PR #190's committed run (`auto/switch-kexo-port`, job 30317123): the same stimulus-skip contributor with the herding copula, the same one-hot-k switch head and the same current-contribution punisher. Its config differs from PR #186's arm A **only** in the switch model. Its `per_round.parquet` is copied into this branch byte-identically (sha256 `d94f9a9b…b471f`), so the merge of the two branches is a no-op on that path.

The copula-**off** cell needed a new simulation, and I judged it worth the cluster time because it is the headline row the whole verdict rests on. It is PR #186's arm B with only the switch model swapped: job **30317620**, 1:12, exit `0:0`. It would not run in either existing tree — this branch's lineage lacks `aimanager.generic.group_vnode` (job 30317585 failed on the import) and PR #186's lineage builds a 23-feature switch input where the one-hot-k head wants 39 (job 30317610 failed on the matmul). Only PR #190's port has both, so I built my own isolated dir `~/repros/ai-runs/head-diag-cat` from PR #186's tree with PR #190's `src/` overlaid, read-only from that worktree; neither sibling's worktree nor cluster directory was written to. The shared artifacts are byte-identical across the lineages (`switch_exodus_k_onehot` sha256 `28dd4b40…c820d`, punisher sha256 `2ad6653e…c820d9`).

Two independent checks that this tree measures the same thing as PR #186. First, re-running PR #190's own config in it reproduces their committed parquet **bit-identically** in contribution, punishment, agent_group and common_good (job 30317648). Second, the human teacher-forced pass under the bare trunk returns Var(E[c|hist]) **27.933** and Var(residual) **11.794** against PR #186's quoted 27.93 and 11.79, and the CG ratio 0.848 against 0.848.

| arm | switch head | copula | Var(E[c\|hist]) | ratio | retention [95% CI] |
|---|---|---|---|---|---|
| skip A (PR #186) | joint_exodus | on | 23.88 | 0.811 | 0.855 |
| **e_skip_kexo** (PR #190) | **k_onehot** | on | **25.11** | 0.828 | **0.899** [0.739, 1.127] |
| c_infl | k_onehot | on | 22.87 | 0.800 | 0.926 [0.790, 1.106] |
| d_kexo | k_onehot | on | 18.51 | 0.804 | 0.793 [0.674, 0.952] |
| skip B (PR #186) | joint_exodus | off | 18.88 | 0.785 | 0.676 |
| **e_skip_kexo_rho0** (new) | **k_onehot** | off | **19.06** | 0.778 | **0.682** [0.547, 0.847] |
| c_infl_rho0 | k_onehot | off | 16.51 | 0.719 | 0.668 [0.575, 0.802] |
| d_kexo_rho0 | k_onehot | off | 13.30 | 0.728 | 0.570 [0.490, 0.683] |

The switch model is worth **+0.006** in retention on the copula-off cell (0.676 -> 0.682) and **+0.044** on the copula-on cell (0.855 -> 0.899).

Per round block the matched copula-off categorical arm keeps diverging late — 3.41 -> 4.39 -> 4.73 — where both Gaussian copula-off arms stall or reverse (3.39 -> 4.33 -> 4.17 inflated, 3.35 -> 3.76 -> 3.82 v2). Human is 4.19 -> 5.54 -> 6.09.
...
4. **The confound has now been measured, and it changes nothing on the headline row** (§3.7). Matching the switch head moves the categorical copula-off retention from 0.676 to **0.682**, a shift of +0.006 — two orders of magnitude smaller than the gap it was supposed to explain away. The matched copula-off ordering is categorical 0.682 > inflated 0.668 > v2 0.570, identical to the unmatched one. The confound is larger on the copula-on cell (+0.044, 0.855 -> 0.899), which is where one would expect a switch head that changes group composition to matter most, and even there it does not overturn anything: matched copula-on is inflated 0.926, categorical 0.899, v2 0.793, with the first two bootstrap intervals almost entirely overlapping ([0.790, 1.106] and [0.739, 1.127]). The one honest nuance the matched cell adds is that **on the copula-on cell the inflated head is nominally ahead of the categorical**, by an amount well inside noise.

   Independently, the confound was never present in §3.6 at all, which runs the three contribution models over the same human states with no switch model in the loop, and that probe orders the heads the same way. So three measurements — confounded closed-loop, matched closed-loop, and unconfounded gain — agree.
...
7. **Contraction is a late-episode failure, not a level offset, and the matched arm sharpens where the heads part company** (§3.5, §3.7). All three copula-off arms start similarly in rounds 1-8 (categorical 3.41, inflated 3.39, v2 3.35, against human 4.19). They then separate: the categorical arm keeps diverging through the last block (4.39 -> **4.73**) while both Gaussian arms stall or reverse (4.33 -> 4.17 inflated, 3.76 -> 3.82 v2). Human keeps climbing to 6.09. So the head difference is not a uniform scaling of spread — it is specifically that the Gaussian heads stop accumulating between-group divergence after round ~16 and the categorical head does not. That is the opposite of the failure mode the hypothesis predicted for the categorical head, and it is the clearest single piece of evidence against it.

   All three still fall well short of the human trajectory, so the shared deficit remains. Whatever generates persistent between-group divergence in the human data is a slow, accumulating process; the copula supplies a crude version of it (a per-episode group latent) and no per-round emission head of any shape supplies much of it. This is consistent with Note 5: the missing ingredient is *persistence across rounds*, a property of the latent, not of the shape of the per-round conditional.

## 5. Verdict

**This kills the combined head as motivated.** The specific argument for building it — that an ordinal location-scale emission would hold state spread where 21 free logits collapse — is contradicted by every measurement taken. The categorical head extrapolates better off-manifold (gain 0.95-1.02 against 0.85-0.96, and it is the only one that does not sag at the extremes); on the fully switch-matched copula-off row it retains more closed-loop state spread (retention **0.682** against 0.668 and 0.570, Var(E) 19.06 against 16.51 and 13.30); and it is the only copula-off arm that keeps diverging through the last round block rather than stalling. The evidence is not mixed on the question as posed; it is one-directional.

The one place the Gaussian side is nominally ahead is the copula-**on** matched cell (inflated 0.926 against categorical 0.899), by a margin well inside overlapping bootstrap intervals. That is a tie, not a win, and it is not the row the architecture argument rested on.
...
- **Do not re-run the head comparison.** Both the confounded closed-loop measurement and the unconfounded gain probe are in this branch with their scripts; `scripts/data_analysis/head_state_spread.py` regenerates every number from committed inputs in five stages (`--teacher-force`, `--gain`, `--gnn-tf` and `--gnn-gain` on Raven, `--analyse`).
- **The switch-model confound is closed** (§3.7); do not re-open it. Both matched cells exist, the matched copula-off row moved the categorical retention by +0.006, and the matched arms are reproducible from `GNN_ARMS` in the script. Reproducing the copula-off arm needs the hybrid tree described in §3.7 (`~/repros/ai-runs/head-diag-cat`: PR #186's tree with PR #190's `src/` overlaid) because neither lineage alone can run that combination; once PR #190 merges, its own tree will run it directly and the hybrid becomes unnecessary.
