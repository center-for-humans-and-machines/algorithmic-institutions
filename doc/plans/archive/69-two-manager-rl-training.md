...
while not done:
    ...
    # Replay stores the full per-group reward for diagnostics; the loss
    # in manager.update() slices to rl_group_id internally.
    replay.add(prev_state, rl_action, reward, state)
# ... in manager.update():
#   loss = smooth_l1_loss(current_q_group[:, rl_group_id], expected_q_rl)
```
...
| 3 | `src/aimanager/rl_manager.py` (manager.update call) + `src/aimanager/manager/manager.py::update()` | Pass `rl_group_id` into `manager.update(...)` as a new kwarg. In `manager.py::update()`, when `rl_group_id` is set, slice `current_q_group`, `next_v`, and `reward` to that group's index before the Huber loss, so the policy is trained only on its own group's TD error. Replay buffer keeps storing all groups' rewards (free, useful for diagnostics + sum/avg comparison plots). | new code |
...
- [x] Step 3: pass `rl_group_id` to `manager.update()`; slice Huber loss to that group inside `manager/manager.py::update()`.
