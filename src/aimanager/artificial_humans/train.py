...
# --------------------------------------------------------------------------- #
# The joint exodus objective (plan step 5)
# --------------------------------------------------------------------------- #
#
...
# pairs. And it would introduce (k_0, k_1) configurations -- (0,6), (1,3),
# (2,4), ... -- that the simulation, which has no timeouts, can never
# produce. Dropping is also what the predecessor's oracle resampled
# (complete pairs only), so the head is fitted on the same object whose
# resample set this experiment's ceiling.
...
def joint_exodus_counts(y, mask, agent_group, batch, *, n_batch):
    ...
    Returns:
        ...
        head's own pooling order. `pool_by_group` accumulates the mask
        weights in the embedding's dtype, so its count channel comes back as
        floats and is rounded here exactly as `JointExodusHead.forward`
        rounds it; the raw return cannot be used as an index.
    """
