...
class ArtificialHumanEnv:
    ...
    def reset_state(self):
        state = {
            ...
            "contributions": th.zeros(
                self.n_agents, dtype=th.int64, device=self.device
            ),
    ...
    def calc_common_good(contributions, punishments, valid):
        return (contributions[valid] * 1.6 - punishments[valid]).mean() * th.ones_like(
            punishments, dtype=th.float
        )
    ...
    def calc_contributions(self):
        ...
        contributions = self.artifical_humans.predict_one(
            encoded[0], reset_rnn=self.round_number[0] == 0
        )[0]
        ...
        self.contributions[~self.valid] = self.artifical_humans.default_values[
            "contributions"
        ]
    ...
    def punish(self, punishments):
        ...
        self.common_good = self.calc_common_good(
            self.contributions, self.punishments, self.valid
        )
        return self.state
    ...
    def step(self):
        ...
        if self.round_number[0] == (self.episode_steps):
            ...
            self.calc_contributions()
            reward = self.contributions.to(th.float) * 1.6 - self.prev_punishments.to(
                th.float
            )
            done = False
        self.payoffs = reward
