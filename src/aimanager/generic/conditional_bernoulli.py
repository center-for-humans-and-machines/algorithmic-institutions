...
(``notes/autoresearch_log/switch-joint-exodus.md``, plan step 3). The round
head (``generic/joint_exodus.py``) decides the pair ``(m_0, m_1)`` of how many
leave each group; this module decides which ones, conditioning on the
existing per-agent head's probabilities ``p_i`` so that the member the model
thinks is most likely to leave is also the one most likely to be picked.
...
and rounds, many such calls) have differing ``k`` and ``m``. Rather than a
flat, index-gathered layout (the ``cell_id`` style ``copula.py`` uses for its
per-cell latent), this module fixes every row's WIDTH at ``MAX_GROUP_SIZE``
and marks which of those 8 slots are real members with a boolean ``mask``
(``None`` meaning "all 8 are real"), mirroring the ``mask`` convention
``joint_exodus.pool_by_group`` already uses for valid deciders. A row's ``k``
is simply ``mask.sum(-1)`` and can differ freely from its neighbours; a fully
merged / fully emptied group (``k == 0``) is just an all-``False`` row, not a
separate code path. This keeps every call a single batched tensor op with no
Python loop over groups, and it composes directly with
``joint_exodus.pool_by_group``'s own per-group masking.
...
imports and is unit-testable on macOS, mirroring ``generic/copula.py`` and
``generic/joint_exodus.py``.
"""
...
#: (or a padded slot, see below) from producing a literal ``-inf``.
_EPS = 1e-12
...
def conditional_bernoulli_log_prob(p, m, *, mask=None, max_group_size=MAX_GROUP_SIZE):
    ...
    # trap entirely; correctness is unaffected because `valid` below throws
    # out every subset that would have included one of them anyway.
...
def sample_conditional_bernoulli(p, m, *, mask=None, max_group_size=MAX_GROUP_SIZE):
    ...
    for the legacy independent switch draw (``th.multinomial(arr, 1)``).
    This is deliberately NOT ``copula.py``'s inverse-CDF construction: that
    module fixes its RNG consumption to two ``randn`` draws per call so an
    AR(1) latent composes predictably across calls with different cell
    layouts. This sampler carries no state between calls (each group/round is
    independent once ``m`` is known), so that contract has nothing to buy
    here, and a single categorical draw over the enumerated subsets is both
    simpler and exact.
    """
