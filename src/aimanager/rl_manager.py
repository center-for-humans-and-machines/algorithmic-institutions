...
def run_batch(
    ...
    for round_number in count():
        ...
        metrics["sampling"] = "greedy" if on_policy else manager.behaviour_label
...
def train_manager(config: dict, labels=None, data_dir: str = None):
    ...
    # The policy-shape rows and this arm's own noise diagnostics. New names,
    # never a rename of a contract metric into another arm's slot. The union
    # over records, not the first record: only behaviour rollouts carry the
    # noise keys, only two-manager runs carry the shape keys.
    extra = {
        k
        for m in metrics_list
        for k in m
        if k.startswith("rpa_") or k.startswith("param_noise_")
    }
    value_vars.extend(sorted(extra))
