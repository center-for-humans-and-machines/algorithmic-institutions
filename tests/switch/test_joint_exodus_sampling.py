"""Tests for the two-stage joint exodus draw in the simulation path (step 6).

Runs locally on macOS with plain pytest::

    PYTHONPATH=$PWD/src python -m pytest tests/switch/test_joint_exodus_sampling.py -q
...
What is under test (notes/autoresearch_log/switch-joint-exodus-gmlp.md, step
6):
...
* head OFF -- ``predict_independent(sample=True)`` is bitwise identical to the
  ...
  the control run of step 11, which re-runs the parent's own config and
...
There is no firing-schedule parameter on this branch (maintainer ruling, note
5 of the log): the firing condition is ``(round + 1) % switch_every == 0``
alone, so with ``switch_every`` 4 all five decision rounds 3, 7, 11, 15, 19
fire and no round is exempt.
"""
...
def _install_pyg_stand_ins():
    ...
    output can tell which implementation was exercised -- with the caveat of
    note 18: in a shared pytest process whichever sibling suite imports first
    is the one that installs them.
    """
    try:
...
def make_model(seed=0, **kwargs):
    """Small switch-shaped model, matching tests/switch/test_joint_exodus_graph.py."""
    th.manual_seed(seed)
    model = GraphNetwork(
        y_levels=2,
        y_name="does_switch",
...
def legacy_predict(model, data, edge_index=None, reset_rnn=True):
    """``predict_independent(sample=True)`` as the pre-change code expressed
    it, copied out, so the head-off identity check does not lean on the code
    under test -- the same device the sibling suites use."""
...
# --------------------------------------------------------------------------- #
@pytest.mark.parametrize("round_number", [0, NON_DECISION_ROUND, DECISION_ROUND, 19])
def test_head_off_sampling_is_bitwise_identical_including_rng(round_number):
    """The control run of step 11 re-runs the parent's own config and requires
    a bit-identical per_round.parquet. The parent's switch artifact carries no
...
def test_a_legacy_artifact_has_no_switch_every_and_still_loads():
    """An artifact saved before this change carries none of the three joint
    keys, and must load with the head absent and sample as it always did."""
    model = make_model(seed=7)
    ...
    (got_pred, _), got_rng = run_seeded(
        lambda: legacy.predict_independent(data, sample=True, edge_index=edge_index)
    )
    assert th.equal(got_pred, ref_pred)
    assert th.equal(got_rng, ref_rng)


# --------------------------------------------------------------------------- #
# 2. head ON, NON-decision round -- still no extra RNG
# --------------------------------------------------------------------------- #
@pytest.mark.parametrize("round_number", [0, 1, 2, 4, 5, 6, 18, 20])
...
def test_a_multi_round_call_fires_only_on_its_decision_rounds():
    """The simulation calls the predictor one round at a time, but the method
    loops over the round axis, and the gate is per round: a four-round window
    starting at round 0 contains exactly one decision round, so it must cost
    the one per-agent draw plus one pair and one selection -- not four."""
    n_rounds = 4
    model = make_model(
        seed=11, joint_exodus=True, joint_exodus_switch_every=SWITCH_EVERY
    )
    data = make_data(n_rounds=n_rounds)
    data["round_number"] = (
        th.arange(n_rounds, dtype=th.int64)
        .reshape(1, 1, n_rounds)
        .expand(2, N_AGENTS, n_rounds)
        .contiguous()
    )
    edge_index = model.create_fully_connected(N_AGENTS, n_batch=2)
    th.manual_seed(SEED)
...
def test_the_leavers_are_members_of_the_group_the_count_was_drawn_for(
    ...
    model = make_model(
        seed=17, joint_exodus=True, joint_exodus_switch_every=SWITCH_EVERY
    )
...
def test_no_switchers_when_the_pair_is_zero_zero():
    ...
    model = make_model(
        seed=19, joint_exodus=True, joint_exodus_switch_every=SWITCH_EVERY
    )
    pin_pair(model, 0, 0)
...
def test_a_fully_merged_round_samples_without_error(groups, k_0, k_1):
    ...
    model = make_model(
        seed=23, joint_exodus=True, joint_exodus_switch_every=SWITCH_EVERY
    )
    data = make_data(n_batch=2, round_number=DECISION_ROUND, groups=groups)
    edge_index = model.create_fully_connected(N_AGENTS, n_batch=2)
...
def test_the_whole_group_can_leave_at_once():
    ...
    model = make_model(
        seed=29, joint_exodus=True, joint_exodus_switch_every=SWITCH_EVERY
    )
...
def test_sample_false_never_reaches_the_joint_path():
    """``sample=False`` is the argmax readout used by evaluation code; it has
    no RNG and must stay the per-agent argmax."""
    model = make_model(
        seed=31, joint_exodus=True, joint_exodus_switch_every=SWITCH_EVERY
    )
    data = make_data(round_number=DECISION_ROUND)
    ...
    th.manual_seed(SEED)
    with CountingMultinomial() as counter:
        pred, proba = model.predict_independent(
...
def test_switch_every_requires_the_head():
    with pytest.raises(AssertionError, match="only meaningful"):
        make_model(joint_exodus=False, joint_exodus_switch_every=SWITCH_EVERY)
